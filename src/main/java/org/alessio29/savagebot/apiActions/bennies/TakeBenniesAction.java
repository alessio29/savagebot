...
public class TakeBenniesAction implements IBotAction {
    ...
    public CommandExecutionResult doAction(IMessageReceived message, String[] args) {
        ...
        if (bType.equals(BennyType.DEADLANDS)) {
            if (args.length < 2) {
                return new CommandExecutionResult("Command syntax: use <CharName> <bennyColor>", args.length + 1);
            }
            ...
            return new CommandExecutionResult("Character " + charName + " used "+bennyColor+" benny.", args.length + 1);
        } else {
            if (args.length < 1) {
